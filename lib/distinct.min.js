...
else _results.push(void 0)}return _results};distinct.set=function(object,path,value){var lastPiece,lastSlash,piece,_i,_len,_ref;lastSlash=path.lastIndexOf("/");lastPiece=path.substring(lastSlash+1);_ref=path.substring(0,lastSlash).split("/");for(_i=0,_len=_ref.length;_i<_len;_i++){piece=_ref[_i];object=object[piece];if(!(object!=null))return false}object[lastPiece]=value;return true};distinct.remove=function(object,path){var lastPiece,lastSlash,piece,_i,_len,_ref;lastSlash=path.lastIndexOf("/");lastPiece=
path.substring(lastSlash+1);_ref=path.substring(0,lastSlash).split("/");for(_i=0,_len=_ref.length;_i<_len;_i++){piece=_ref[_i];object=object[piece];if(!(object!=null))return false}if(object.hasOwnProperty(lastPiece)){delete object[lastPiece];return true}return false};distinct.diff=function(oldObject,newObject,changesUpTheTree){var results;if(!(oldObject!=null)||!(newObject!=null))throw new Error("oldObject or newObject can't be null");results=[];distinct._diff(oldObject,newObject,changesUpTheTree,
"",0,results);results.sort(function(a,b){if(a.depth>b.depth)return 1;if(a.depth<b.depth)return-1;if(a.path>b.path)return 1;if(a.path<b.path)return-1;return 0});return results};distinct._diff=function(oldObject,newObject,changesUpTheTree,selectorPrefix,depth,results){var key,path,somethingChanged,value;somethingChanged=false;for(key in oldObject){value=oldObject[key];path=selectorPrefix+"/"+key;if(!newObject.hasOwnProperty(key)){results.push({path:path,diffType:"delete",depth:depth});somethingChanged=
true}else if(newObject[key]===Object(newObject[key]))if(oldObject[key]===Object(oldObject[key])){if(!JsTraverse._diff(oldObject[key],newObject[key],changesUpTheTree,path,depth+1,results&&changesUpTheTree)){results.push({path:path,diffType:"change",depth:depth,oldValue:oldObject[key],newValue:newObject[key]});somethingChanged=true}}else{results.push({path:path,diffType:"change",depth:depth,oldValue:oldObject[key],newValue:newObject[key]});somethingChanged=true}else if(newObject[key]!==oldObject[key]){results.push({path:path,
diffType:"change",depth:depth,oldValue:oldObject[key],newValue:newObject[key]});somethingChanged=true}}for(key in newObject){value=newObject[key];if(!oldObject.hasOwnProperty(key)){results.push({path:selectorPrefix+"/"+key,diffType:"add",depth:depth});somethingChanged=true}}return somethingChanged};distinct.applyDiff=function(object,diffArray,eventObject){var diff,_i,_len,_results;_results=[];for(_i=0,_len=diffArray.length;_i<_len;_i++){diff=diffArray[_i];switch(diff.diffType){case "add":if(distinct.set(object,
diff.path,diff.newValue&&eventObject!=null))_results.push(eventObject.trigger("add:"+diff.path,diff));else _results.push(void 0);break;case "delete":if(distinct.remove(object,diff.path&&eventObject!=null))_results.push(eventObject.trigger("delete:"+diff.path,diff));else _results.push(void 0);break;default:if(diff.hasOwnProperty("newValue"))if(distinct.set(object,diff.path,diff.newValue&&eventObject!=null))_results.push(eventObject.trigger("change:"+diff.path,diff));else _results.push(void 0);else _results.push(void 0)}}return _results}}).call(this);
